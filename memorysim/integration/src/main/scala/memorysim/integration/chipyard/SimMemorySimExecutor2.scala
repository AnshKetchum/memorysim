...
/** Improved SimMemorySimExecutor
  ...
  *   - Assumes beatBytes == dataBits/8 (common case). If your test uses sub-beat sizes we can extend this to handle
  *     those by byte-assembling beats.
  *
  * This file uses plain printf(...) format strings (no p interpolator).
  */
class SimMemorySimExecutor2(memSize: BigInt, lineSize: Int, memBase: BigInt, params: AXI4BundleParameters, chipId: Int)
    extends Module {
