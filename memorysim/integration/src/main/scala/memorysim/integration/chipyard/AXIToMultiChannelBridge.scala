...
class AXIToMultiChannelBridge(
  ...
  switch(wState) {
    is(sWIdle) {
      when(io.axi.aw.fire) {
        ...
        printf(
          "[AXI-Bridge] AW.fire id=%d addr=%x len=%d size=%d\n",
          io.axi.aw.bits.id,
          io.axi.aw.bits.addr,
          io.axi.aw.bits.len,
          io.axi.aw.bits.size
  ...
  switch(rState) {
    is(sRIdle) {
      when(io.axi.ar.fire) {
        ...
        rState := sRIssue
        printf(
          "[AXI-Bridge] AR.fire id=%d addr=%x len=%d size=%d\n",
          io.axi.ar.bits.id,
          io.axi.ar.bits.addr,
          io.axi.ar.bits.len,
          io.axi.ar.bits.size
  ...
  memSys.io.in.bits.addr := Mux(mem_req_is_write, store_addr, read_addr + (read_issued << read_size))
  ...
  when(memSys.io.in.fire) {
    printf(
      "[AXI-Bridge] MemSys req: wr=%d rd=%d addr=%x data=%x\n",
      memSys.io.in.bits.wr_en,
      memSys.io.in.bits.rd_en,
...
class RealisticMemorySimExecutor(
  ...
  val bridge = Module(new AXIToMultiChannelBridge(memSize, lineSize, memBase, params, memParams, localConfig, chipId))
  ...
  printf(
    "[RealisticMemSim] Active ranks: %d, Req queue: %d, Resp queue: %d\n",
    io.activeRanks,
    io.reqQueueCount,
    io.respQueueCount
  )
