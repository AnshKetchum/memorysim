...
class SimMemorySimExecutor(
  ...
  when (arb.io.out.fire) {
    ...
    when (arb.io.out.bits.isRead) {
      ...
      printf("[MemorySim] AR(arb) id=%d addr=%x len=%d size=%d\n",
        arb.io.out.bits.id, arb.io.out.bits.addr, arb.io.out.bits.len, arb.io.out.bits.size)
    } .otherwise {
      ...
      printf("[MemorySim] AW(arb) id=%d addr=%x len=%d size=%d\n",
        arb.io.out.bits.id, arb.io.out.bits.addr, arb.io.out.bits.len, arb.io.out.bits.size)
    }
  }
  ...
  switch (wState) {
    ...
    is (sWCollect) {
      when (io.axi.w.fire) {
        ...
        val newBeat = Mux(io.axi.w.bits.strb === fullMask,
                          io.axi.w.bits.data, {
  ...
  // Debug prints
  printf("Bridge rState=%d wState=%d transActive=%d isRead=%d mem.in.vld=%d mem.in.rdy=%d mem.out.vld=%d mem.out.rdy=%d\n",
    rState, wState, transactionActive, currentIsRead, memWrapper.io.in.valid, memWrapper.io.in.ready, memWrapper.io.out.valid, memWrapper.io.out.ready)
