...
/** MultiChannel-backed SimMemorySimExecutor
  *
  * Drop-in replacement for the original SimMemorySimExecutor that uses
  * MultiChannelSystem as the backing memory instead of SyncReadMem.
  * Provides realistic memory controller timing while maintaining the same AXI4 interface.
  *
...
class SimMemorySimExecutor(
  ...
  val memParams = SingleChannelMemoryConfigurationParams(
    memConfiguration = MemoryConfigurationParameters(
      ...
      memoryQueueSize = 256    // WARNING: Default queue size
    ),
    ...
    trackPerformance = false // WARNING: Performance tracking disabled by default
  )
    ...
    is(sWWaitResp) {
      ...
      when(memSys.io.out.fire && memSys.io.out.bits.wr_en) {
        ...
        when(write_responses_pending === 1.U) {
          wState := sWResp
          printf("[MemorySim] All write responses received, sending B response\n")
        }
      }
    }
    is(sWResp) {
  ...
  switch(rState) {
    ...
    is(sRCollect) {
      ...
      when(memSys.io.out.fire && memSys.io.out.bits.rd_en) {
        ...
        printf("[MemorySim] Read response %d: data=%x addr=%x\n", 
               beat_index, memSys.io.out.bits.data, memSys.io.out.bits.addr)
      }
      ...
      when(io.axi.r.fire) {
        ...
        printf("[MemorySim] R.fire id=%d data=%x last=%d beats_sent=%d\n",
          read_id, read_buffer(read_beats_sent), 
          (read_beats_sent === (read_count - 1.U)).asUInt, read_beats_sent + 1.U)
  ...
  memSys.io.in.bits.addr := Mux(mem_req_is_write, 
    store_addr,
    read_addr + (read_beats_issued << read_size))
  ...
  printf("[MemorySim] Active ranks: %d, Req queue: %d, Resp queue: %d\n",
    io.debug.activeRanks, io.debug.reqQueueCount, io.debug.respQueueCount)
}